export default {
  show_in_finder: 'Show in Finder',
  save_as: 'Save As…',
  image: 'Image',
  images: 'Images',
  video: 'Video',
  videos: 'Videos',
  file: 'File',
  files: 'Files',
  message: 'Message',
  shared_folder: 'Shared folder',
  folder_card_items: '{n} items',
  share_expires_on: 'Expires {date}',
  share_expired: 'Expired',
  send_to_phone_clipboard: 'Send to phone clipboard',
  clipboard_text: 'Clipboard text',
  my_phone: 'My phone',
  me: 'Me',
  this_device: 'This device',
  chat_input_hint: 'What is in your mind?',
  forward_message: 'Forward message',
  copy_text: 'Copy text',
  delete_message: 'Delete message',
  chat_info: 'Chat info',
  clear_messages: 'Clear messages',
  clear_messages_confirm: 'Are you sure you want to clear all messages? This action cannot be undone.',
  messages_cleared: 'Messages cleared!',
  channels: 'Channels',
  create_channel: 'Create channel',
  channel_name: 'Channel name',
  rename_channel: 'Rename channel',
  delete_channel: 'Delete channel',
  delete_channel_confirm: 'Deleting this channel will also delete all related messages. This action cannot be undone.',
  delete_device: 'Delete device',
  delete_peer_warning: 'Deleting this device will also delete all related chat messages. This action cannot be undone.',
  leave_channel: 'Leave channel',
  leave_channel_confirm: 'Are you sure you want to leave this channel? You will no longer receive messages.',
  channel_info: 'Channel info',
  channel_members: 'Members',
  channel_creator: 'Creator',
  add_member: 'Add member',
  invite: 'Invite',
  remove_member: 'Remove member',
  waiting_for_confirmation: 'Waiting for confirmation',
  discovered_via_bluetooth: 'Discovered via Bluetooth',
  discovered_via_lan: 'Discovered via Wi-Fi',
  accept_invite: 'Accept',
  decline_invite: 'Decline',
  channel_invite: 'Channel invite',
  channel_invite_desc: 'You have been invited to join "{name}".',
  new_chat: 'New chat',
  config: 'Config',
  api_key: 'API key',
  no_api_key: 'No api key',
  app_files: 'App files',
  send_failed: 'Failed to send',
  delivery_status: 'Delivery Status',
  delivery_status_summary: '{delivered} / {total} delivered',
  delivery_failed: 'Error',
  nearby_devices: 'Nearby Devices',
  scanning_devices: 'Scanning devices...',
  devices: 'Devices',
  scanning: 'Scanning…',
  no_devices_found: 'No nearby devices found.',
  pair_request_from: 'Pairing request from {name}',
  paired_devices: 'Paired Devices',
  unpaired_devices: 'Unpaired Devices',
  device_identity: 'Device Identity',
  device_name: 'Device Name',
  client_id: 'Client ID',
  public_key: 'Public Key',
  pair_request: 'Pairing Request',
  pairing_request_message: 'Do you want to pair with device "{name}"?',
  pairing_request_hint: 'After pairing, you can exchange messages and files between both devices.',
  allow: 'Allow',
  deny: 'Deny',
  pairing: 'Pairing…',
  pair_success: 'Paired successfully!',
  pair_failed: 'Pairing failed',
  pair: 'Pair',
  pair_device: 'Pair device',
  pairing_with: 'Pair with {name}',
  pairing_access_prompt: 'Choose how to connect to this device',
  pairing_chat_only: 'Chat only',
  pairing_chat_only_hint: 'Exchange messages and files',
  pairing_full_control: 'Full control',
  pairing_full_control_hint: 'Manage this device and exchange messages and files',
  pairing_start: 'Continue',
  pairing_connecting: 'Sending pair request…',
  pairing_waiting: 'Waiting for response…',
  pairing_timed_out: 'Pairing timed out — the other device did not respond.',
  pairing_cancelled: 'Pairing cancelled.',
  found_devices: '{count} device(s) found',
  rescan: 'Rescan',
  pending: 'Pending',
  unpair: 'Unpair',
  paired: 'Paired',
  unpaired: 'Unpaired',
  channel_left_notice: 'You have left this channel and can no longer send messages.',
  channel_kicked_notice: 'You were removed from this channel and can no longer send messages.',
}
