export default {
  show_in_finder: 'Mostra nel Finder',
  save_as: 'Salva come…',
  image: 'Immagine',
  video: 'Video',
  file: 'File',
  message: 'Messaggio',
  shared_folder: 'Cartella condivisa',
  folder_card_items: '{n} elementi',
  share_expires_on: 'Scade {date}',
  share_expired: 'Scaduto',
  my_phone: 'Il mio telefono',
  me: 'Me',
  this_device: 'Questo dispositivo',
  chat_input_hint: 'Cosa hai in mente?',
  forward_message: 'Inoltra messaggio',
  copy_text: 'Copia testo',
  delete_message: 'Elimina messaggio',
  chat_info: 'Info chat',
  clear_messages: 'Cancella messaggi',
  clear_messages_confirm: 'Sei sicuro di voler cancellare tutti i messaggi? Questa azione non può essere annullata.',
  messages_cleared: 'Messaggi cancellati!',
  new_chat: 'Nuova chat',
  ai: 'IA',
  config: 'Configurazione',
  api_key: 'Chiave API',
  no_api_key: 'Nessuna chiave API',
  app_files: "File dell'app",
  send_to_phone_clipboard: 'Invia agli appunti del telefono',
  clipboard_text: 'Testo degli appunti',
  devices: 'Dispositivi',
  channels: 'Canali',
  create_channel: 'Crea canale',
  channel_name: 'Nome del canale',
  rename_channel: 'Rinominare il canale',
  delete_channel: 'Elimina canale',
  delete_channel_confirm: 'Sei sicuro di voler eliminare questo canale? Tutti i messaggi andranno persi.',
  delete_device: 'Elimina dispositivo',
  delete_peer_warning: 'Eliminare questo dispositivo eliminerà anche tutti i messaggi di chat. Questa azione non può essere annullata.',
  leave_channel: 'Lascia il canale',
  leave_channel_confirm: 'Sei sicuro di voler lasciare questo canale?',
  channel_info: 'Informazioni sul canale',
  channel_members: 'Membri',
  channel_creator: 'Creatore',
  add_member: 'Aggiungi membro',
  invite: 'Invita',
  remove_member: 'Rimuovi membro',
  waiting_for_confirmation: 'In attesa di conferma',
  accept_invite: 'Accettare',
  decline_invite: 'Declino',
  channel_invite: 'Invito al canale',
  channel_invite_desc: 'Sei stato invitato a unirti a "{name}".',
  send_failed: 'Invio fallito',
  delivery_status: 'Stato consegna',
  delivery_status_summary: '{delivered} / {total} consegnati',
  delivery_failed: 'Errore',
  pair_request: 'Richiesta di associazione',
  pairing_request_message: 'Vuoi accoppiarti con il dispositivo "{name}"?',
  pairing_request_hint: 'Dopo l\'associazione, puoi scambiare messaggi e file tra i due dispositivi.',
  pair_device: 'Associa dispositivo',
  pairing_with: 'Associa con {name}',
  pairing_access_prompt: 'Scegli come connetterti a questo dispositivo',
  pairing_chat_only: 'Solo chat',
  pairing_chat_only_hint: 'Scambia messaggi e file',
  pairing_full_control: 'Controllo completo',
  pairing_full_control_hint: 'Gestisci questo dispositivo e scambia messaggi e file',
  login_device_hint: 'Dispositivo connesso · controllo completo',
  pairing_start: 'Continua',
  allow: 'Consenti',
  deny: 'Nega',
  pending: 'In attesa',
  unpair: 'Disaccoppia',
  paired: 'Accoppiato',
  unpaired: 'Non accoppiato',
  channel_left_notice: 'Hai abbandonato questo canale e non puoi più inviare messaggi.',
  channel_kicked_notice: 'Sei stato rimosso da questo canale e non puoi più inviare messaggi.',
}
