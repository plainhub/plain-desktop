export default {
  upload_failed: 'Yükleme başarısız',
  release_to_send_files: 'Dosyayı göndermek için bırakın',
  storage: 'Depolama',
  files: 'Dosyalar',
  storage_free_total: '{free} kullanılabilir / {total}',
  current_path: 'Geçerli yol',
  select_folder: 'Klasör seç',
  create_folder: 'Klasör oluştur',
  upload_files: 'Dosyaları yükle',
  upload_folder: 'Klasör yükle',
  save_to: 'Kaydet',
  change_save_location: 'Kayıt konumunu değiştir',
  upload: 'Yükle',
  download: 'İndir',
  download_individually: 'Tek tek indir',
  download_as_zip: 'ZIP olarak indir',
  downloaded_to: '{dir} klasörüne indirildi',
  show_in_folder: 'Klasörde göster',
  download_failed: 'İndirme başarısız',
  recent_folders: 'Son klasörler',
  show_hidden: 'Gizli öğeleri göster',
  select_mode: 'Seçim modu',
  ext: 'Uzantı',
  file_size: 'Dosya boyutu',
  invalid_file_signature: 'Geçersiz dosya imzası',
  upload_status: {
    created: 'Beklemede',
    pending: 'Yükleniyor',
    saving: 'Kaydediliyor',
    done: 'Yüklendi',
    error: 'Yükleme başarısız oldu',
    uploading: 'Yükleniyor',
    paused: 'Duraklatıldı',
  },
  upload_chunk_failed: 'Bazı parçalar yüklenemedi',
  upload_merge_failed: 'Parçalar birleştirilemedi',
  upload_merge_size_mismatch: 'Birleştirilen dosya boyutu orijinal dosyayla eşleşmiyor',
  upload_auto_paused: 'Tekrarlayan ağ hataları sonrasında yüklemeler otomatik olarak duraklatıldı. Bağlantı geri geldiğinde devam edin.',
  upload_unauthorized: 'Oturum süresi doldu, lütfen tekrar oturum açın',
  deleting: 'Siliniyor…',
  delete_failed_name: '"{name}" silinemedi',
  deleted_n_items: '{n} öğe silindi',
  delete_files: 'Dosyaları sil',
  recents: 'Son kullanılanlar',
  internal_storage: 'Dahili depolama',
  sdcard: 'SD kartı',
  recent_files: 'Son dosyalar',
  usb_storage: 'USB depolama',
  folders: 'Klasörler',
  volumes: 'Birimler',
  app_data: 'Uygulama verileri',
  file_not_found: 'Dosya bulunamadı',
  access_denied: 'Erişim reddedildi',
  failed_to_load_file: 'Dosya yüklenemedi',
  invalid_file_id: 'Geçersiz dosya ID',
  invalid_json_format: 'Geçersiz JSON formatı',
  raw_text: 'Ham metin',
  formatted_view: 'Biçimlendirilmiş görünüm',
  copy_path: 'Yolu kopyala',
  json_n_items: '{n} öğe',
  json_n_keys: '{n} anahtar',
  viewer_source: 'Kaynak',
  viewer_tree: 'Ağaç',
  format: 'Biçimlendir',
  minify: 'Küçült',
  depth: 'Derinlik',
  large_file_virtual: 'Büyük dosya · sanal',
  n_matches: '{n} eşleşme',
  invalid_json_expr: 'Geçersiz ifade',
  json_valid: 'Geçerli JSON',
  json_invalid: 'Geçersiz JSON',
  xml_valid: 'Geçerli XML',
  viewer_ready: 'Yüklendi',
  auto_fix: 'Otomatik onar',
  invalid_json_tree_hint: 'Geçersiz JSON · düzelttikten sonra ağaç görüntülenir',
  n_lines: '{n} satır',
  error_line_col: 'Satır {line}, Sütun {col}',
  doc_preview_title: 'DOC önizleme',
  doc_preview_hint: 'Planlanan: metin çıkarma ve sayfalı gösterim',
  plain_text: 'Düz metin',
  fullscreen: 'Tam ekran',
  exit_fullscreen: 'Tam ekrandan çık',
  delete_file: 'Dosyayı sil',
  invalid_file_path: 'Geçersiz dosya yolu',
  file_types: 'Dosya türleri',
  supports_online_preview: 'Çevrimiçi önizleme destekleniyor',
}
