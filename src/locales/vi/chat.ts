export default {
  show_in_finder: 'Hiển thị trong Finder',
  save_as: 'Lưu thành…',
  image: 'Ảnh',
  video: 'Video',
  file: 'Tệp',
  message: 'Tin nhắn',
  shared_folder: 'Thư mục được chia sẻ',
  folder_card_items: '{n} mục',
  share_expires_on: 'Hết hạn {date}',
  share_expired: 'Đã hết hạn',
  my_phone: 'Điện thoại của tôi',
  me: 'Tôi',
  this_device: 'Thiết bị này',
  chat_input_hint: 'Bạn đang nghĩ gì?',
  forward_message: 'Chuyển tiếp tin nhắn',
  copy_text: 'Sao chép văn bản',
  delete_message: 'Xóa tin nhắn',
  chat_info: 'Thông tin chat',
  clear_messages: 'Xóa tin nhắn',
  clear_messages_confirm: 'Bạn có chắc chắn muốn xóa tất cả tin nhắn không? Hành động này không thể hoàn tác.',
  messages_cleared: 'Tin nhắn đã được xóa!',
  new_chat: 'Trò chuyện mới',
  ai: 'Trí tuệ nhân tạo',
  config: 'Cấu hình',
  api_key: 'Khóa API',
  no_api_key: 'Không có khóa API',
  app_files: 'Tệp ứng dụng',
  send_to_phone_clipboard: 'Gửi vào bảng nhớ tạm trên điện thoại',
  clipboard_text: 'Văn bản bảng nhớ tạm',
  devices: 'Thiết bị',
  channels: 'Kênh',
  create_channel: 'Tạo kênh',
  channel_name: 'Tên kênh',
  rename_channel: 'Đổi tên kênh',
  delete_channel: 'Xóa kênh',
  delete_channel_confirm: 'Bạn có chắc chắn muốn xóa kênh này không? Tất cả tin nhắn sẽ bị mất.',
  delete_device: 'Xóa thiết bị',
  delete_peer_warning: 'Xóa thiết bị này cũng sẽ xóa tất cả tin nhắn trò chuyện. Hành động này không thể hoàn tác.',
  leave_channel: 'Rời khỏi kênh',
  leave_channel_confirm: 'Bạn có chắc chắn muốn rời khỏi kênh này không?',
  channel_info: 'Thông tin kênh',
  channel_members: 'Thành viên',
  channel_creator: 'Người tạo',
  add_member: 'Thêm thành viên',
  invite: 'Mời',
  remove_member: 'Xóa thành viên',
  waiting_for_confirmation: 'Đang chờ xác nhận',
  accept_invite: 'Chấp nhận',
  decline_invite: 'Sự suy sụp',
  channel_invite: 'Mời kênh',
  channel_invite_desc: 'Bạn đã được mời tham gia "{name}".',
  send_failed: 'Gửi thất bại',
  delivery_status: 'Trạng thái gửi',
  delivery_status_summary: '{delivered} / {total} đã gửi',
  delivery_failed: 'Lỗi',
  pair_request: 'Yêu cầu ghép nối',
  pairing_request_message: 'Bạn có muốn ghép nối với thiết bị "{name}"?',
  pairing_request_hint: 'Sau khi ghép đôi, bạn có thể trao đổi tin nhắn và tệp giữa hai thiết bị.',
  pair_device: 'Ghép nối thiết bị',
  pairing_with: 'Ghép nối với {name}',
  pairing_access_prompt: 'Chọn cách kết nối với thiết bị này',
  pairing_chat_only: 'Chỉ trò chuyện',
  pairing_chat_only_hint: 'Trao đổi tin nhắn và tệp',
  pairing_full_control: 'Toàn quyền điều khiển',
  pairing_full_control_hint: 'Quản lý thiết bị này và trao đổi tin nhắn, tệp',
  pairing_start: 'Tiếp tục',
  allow: 'Cho phép',
  deny: 'Từ chối',
  pending: 'Đang chờ',
  unpair: 'Hủy ghép nối',
  paired: 'Đã ghép nối',
  unpaired: 'Chưa ghép nối',
  channel_left_notice: 'Bạn đã rời khỏi kênh này và không thể gửi tin nhắn.',
  channel_kicked_notice: 'Bạn đã bị xóa khỏi kênh này và không thể gửi tin nhắn.',
}
