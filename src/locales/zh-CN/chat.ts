export default {
  show_in_finder: '在 Finder 中显示',
  save_as: '另存为…',
  image: '图片',
  video: '视频',
  file: '文件',
  message: '消息',
  shared_folder: '共享文件夹',
  folder_card_items: '{n} 项',
  share_expires_on: '{date} 过期',
  share_expired: '已过期',
  send_to_phone_clipboard: '发送到手机剪贴板',
  clipboard_text: '剪贴板内容',
  my_phone: '我的手机',
  me: '我',
  this_device: '本设备',
  chat_input_hint: '您想说什么？',
  forward_message: '转发消息',
  copy_text: '复制文字',
  delete_message: '删除消息',
  chat_info: '聊天信息',
  clear_messages: '清空消息',
  clear_messages_confirm: '确定要清空所有消息吗？此操作无法撤销。',
  messages_cleared: '消息已清空！',
  channels: '频道',
  create_channel: '创建频道',
  channel_name: '频道名称',
  rename_channel: '重命名频道',
  delete_channel: '删除频道',
  delete_channel_confirm: '确定要删除此频道吗？所有消息将会丢失。',
  delete_device: '删除设备',
  delete_peer_warning: '删除此设备将同时删除所有相关的聊天记录。此操作无法撤销。',
  leave_channel: '离开频道',
  leave_channel_confirm: '确定要离开此频道吗？',
  channel_info: '频道信息',
  channel_members: '成员',
  channel_creator: '创建者',
  add_member: '添加成员',
  invite: '邀请',
  remove_member: '移除成员',
  waiting_for_confirmation: '等待确认',
  discovered_via_bluetooth: '通过蓝牙发现',
  discovered_via_lan: '通过 Wi-Fi 发现',
  accept_invite: '接受',
  decline_invite: '拒绝',
  channel_invite: '频道邀请',
  channel_invite_desc: '您被邀请加入"{name}"。',
  new_chat: '创建会话',
  ai: '人工智能',
  config: '配置',
  api_key: 'API密钥',
  no_api_key: '缺少 API 密钥',
  app_files: '应用文件',
  send_failed: '发送失败',
  delivery_status: '发送状态',
  delivery_status_summary: '{delivered} / {total} 已送达',
  delivery_failed: '错误',
  nearby_devices: '附近设备',
  scanning_devices: '正在搜索设备...',
  devices: '设备',
  scanning: '扫描中…',
  no_devices_found: '未发现附近设备。',
  pair_request_from: '来自 {name} 的配对请求',
  paired_devices: '已配对设备',
  unpaired_devices: '未配对设备',
  device_identity: '设备身份',
  device_name: '设备名称',
  client_id: '客户端 ID',
  public_key: '公钥',
  pair_request: '配对请求',
  pairing_request_message: '是否要与设备"{name}"配对？',
  pairing_request_hint: '配对后，你们可以互发消息和文件。',
  allow: '允许',
  deny: '拒绝',
  pairing: '配对中…',
  pair_success: '配对成功！',
  pair_failed: '配对失败',
  pair: '配对',
  pair_device: '配对设备',
  pairing_with: '与{name}配对',
  pairing_access_prompt: '选择与这台设备的连接方式',
  pairing_chat_only: '仅聊天',
  pairing_chat_only_hint: '互发消息和文件',
  pairing_full_control: '完全控制',
  pairing_full_control_hint: '登录管理设备，并互发消息与文件',
  pairing_start: '继续',
  pending: '待确认',
  unpair: '解除配对',
  paired: '已配对',
  unpaired: '未配对',
  channel_left_notice: '你已离开此频道，无法再发送消息。',
  channel_kicked_notice: '你已被踢出此频道，无法再发送消息。',
}
